...
document.getElementById('inputfuncz').addEventListener('input',(ev) => {
    // `ev.target` is an instance of `MathfieldElement`
    funcinpz=ev.target.getValue('ascii-math');
});
...
function plotgraph() {
  ...
  const exprz = math.compile(funcinpz);
  ...
  const yValues = valt.map(function(t) {
   ...
   const zValues = valt.map(function(t) {
    return exprz.evaluate({
      t: t
    })
  })
...
wrkrng=wrkwdth/60;
  ...
  var plotline = {
    ...
    type: 'scatter3d',
    line: {
      color: 'rgb(216,158,227)',// alt color : rgb(208,115,232)
      width:6
    },
    marker:{
    size:0.1
...
autosize: false,
  ...
  data = [plotline];
    ...
    Plotly.newPlot('plotarea', data,layout,{ displaylogo: false , responsive: true,modeBarButtonsToRemove: ['select2d', 'lasso2d', 'toggleSpikelines', 'autoScale2d', 'hoverClosestCartesian', 'hoverCompareCartesian'] });                   //plotting 
